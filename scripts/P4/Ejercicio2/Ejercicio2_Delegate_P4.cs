...
* Fecha: 18/10/23
* Script que sirve para crear un evento que permite que cuando el cubo colisiona con cualquier objeto que no sean esferas del grupo 1, las esferas 
...
public class Ejercicio2_Delegate_P4 : MonoBehaviour
{
    public delegate void CollisionEvent();
    public event CollisionEvent OnCollideFirstGroup;
    public event CollisionEvent OnCollideOtherType;
    private void OnCollisionEnter(Collision other)
    {
        if (other.gameObject.tag == "sphere_type_1") {
           OnCollideFirstGroup?.Invoke();
        } else {
            OnCollideOtherType?.Invoke();
        }
    }
}
